...
//Removing from the Database
		...
		//public static Rental GetByCopyID(int copyId)
		//{
		//	using (NpgsqlConnection conn = new NpgsqlConnection(Configuration.CONNECTION_STRING))
		//	{
		//		conn.Open();
		//		using (var command = new NpgsqlCommand($"SELECT * FROM {table} WHERE copy_id = @CopyId", conn))
		//		{
		//			command.Parameters.AddWithValue("@CopyId", copyId);
		...
		//			{
		//				reader.Read();
		//				return new Rental((int)reader["copy_id"], (int)reader["client_id"], (DateTime)reader["date_of_rental"], (DateTime)reader["date_of_return"]);
		//			}
		//		}
		//	}
		//	return null;
